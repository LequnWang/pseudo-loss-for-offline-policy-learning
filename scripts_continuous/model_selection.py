...
def select_by_PL(d, alpha=0.1):
    ...
    for param in d["params"]:
        ...
        objective = result["IPW"] + np.sqrt(2 * result["pseudo_loss"] * np.log(4 / alpha) / result["valid_data_size"] / result["h"])
        if best_objective is None or objective < best_objective:
            ...
            best_confidence = np.sqrt(2 * result["pseudo_loss"] * np.log(4 / alpha) / result["valid_data_size"] / result["h"])
                                ...
                                for k in ks:
                                    for h in hs:
                                        for weight_decay in weight_decays:
                                            param_string = "_".join([str(beta), str(k), str(h), str(weight_decay)])
                                            model_identity_string = "_".join(
                                                [data_name, str(data_size), str(estimator), str(run), str(beta), str(k),
                                                 str(h), str(weight_decay)])
                                            if confidence == "None":
                                            ...
                                            try:
                                                with open(result_path, 'rb') as f:
                                                    ...
                                                    results_dict[data_name][oracle][estimator][confidence][data_size][run]["params"][param_string] = result
                                            except:
                                                print(result_path)
                                                continue
